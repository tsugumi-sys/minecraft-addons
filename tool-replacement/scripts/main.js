...
const TOOL_TYPES = [
    ...
    'minecraft:bow', 'minecraft:crossbow', 'minecraft:trident', 'minecraft:fishing_rod', 'minecraft:flint_and_steel', 'minecraft:shears'
];
...
function replaceTool(player, replacementInfo) {
    try {
        ...
        player.sendMessage("§a道具を自動的に持ち替えました！ / Tool automatically replaced!");
...
system.runInterval(() => {
    for (const player of world.getAllPlayers()) {
        try {
            ...
            if (!mainhandItem || !TOOL_TYPES.includes(mainhandItem.typeId)) {
                continue;
            }
            
            const durabilityComponent = mainhandItem.getComponent("durability");
            if (!durabilityComponent) {
                continue;
            }
            
            if (durabilityComponent.damage >= durabilityComponent.maxDurability) {
                const replacementTool = findReplacementTool(player, mainhandItem.typeId);
            ...
            console.warn("Error checking tool durability:", error);
        }
    }
}, 5);
